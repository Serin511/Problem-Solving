...
int main() {
	...
	lct.init(N);
	for (int i=1; i<=N; i++) {
		lct.upd(1, line(A[i], S[i]-A[i]*i), 1, N);
		ans=max(ans, lct.get(1, i, 1, N)-S[i]);
	}
	lct.init(N);
	for (int i=N; i>=1; i--) {
		lct.upd(1, line(A[i], S[i-1]-A[i]*i), 1, N);
		ans=max(ans, lct.get(1, i, 1, N)-S[i-1]);
	}
	printf("%lld\n", ans+val);
	return 0;
}
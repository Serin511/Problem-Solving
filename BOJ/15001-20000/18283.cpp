...
int N; ll H[200010], D[200010]; int s[200010], e[200010], stk[200010], tp;

int main() {
	scanf("%d", &N); lct.init(N*2);
	for (int i=1; i<=N; i++) {
		scanf("%lld", &H[i]);
		while (tp&&H[stk[tp-1]]>H[i]) {
			e[stk[tp-1]]=i-1;
			tp--;
		}
		s[i]=tp?stk[tp-1]+1:1;
	...
	for (int i=1; i<=N; i++) {
		lct.upd(1, line(H[i], H[i]*(1-s[i])), 1, N, 1, e[i]);
		D[i]=max(D[i-1], lct.get(1, i, 1, N));
		printf("%lld ", D[i]);
	}
	puts("");
	return 0;

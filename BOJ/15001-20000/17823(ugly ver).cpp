...
struct node {
	vlm V[2][2]; int sz;
	node(int SZ=0) { sz=SZ; for (register int i=0; i<4; i++) V[(i&2)/2][i&1]=vlm(sz+1, -LINF); }
};
...
void merge(node &r, node &n1, node &n2) {
	...
	for (register int i=0; i<16; i++) {
		...
		vlm V, V1, V2;
		V.eb(R1[s1]+R2[s2]);
		for (register int j=s1; j<e1; j++) V1.eb(R1[j+1]-R1[j]);
		for (register int j=s2; j<e2; j++) V2.eb(R2[j+1]-R2[j]);
		...
		for (register int j=1; j<V.size(); j++) V[j]+=V[j-1];
...
struct Seg {
	...
	void sol(int n, int s, int e) {
		...
		for (register int i=0; i<4; i++) {
			...
			for (register int j=0; j<v[n].size(); j++) {
				while (t+1<=st[n].sz&&st[n].V[x][y][t]<=st[n].V[x][y][t+1]+XX[v[n][j]]) t++;
				im[i].eb(st[n].V[x][y][t]+t*XX[v[n][j]], t);
			}
		}
		for (register int i=0; i<v[n].size(); i++) T[v[n][i]].eb(im[0][i], im[1][i], im[2][i], im[3][i]);
	}
}S;
...
int main() {
	...
	for (register int t=27; t--; ) {
		...
		for (register int i=0; i<Q; i++) {
			auto v=T[i][0];
			for (register int j=1; j<T[i].size(); j++) v=merge(XX[i], v, T[i][j]);
